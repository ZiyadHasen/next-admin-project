...
import { data } from '../data/tableDataUser13';
      ...
      <table className='w-full border-collapse border text-gray-600 border-gray-300'>
        <thead>
          <tr className='bg-transparent'>
            ...
            <th className='border px-4 py-2'>미션 활성 여부</th>
            <th className='border px-4 py-2'>쿠폰명</th>
            <th className='border px-4 py-2'>할인 헤택</th>
          ...
          {currentData.map((item, index) => (
            <tr
              className={`text-center text-sm border border-gray-100 px-4 py-2 ${
                index >= itemsPerPage - 4 ? 'bg-backG' : 'bg-white'
              }`}
              key={item.id}
            >
              <td
                className={`border text-center text-sm border-gray-100 px-4 py-2 ${
                  index >= itemsPerPage - 4 ? 'text-gray-400' : 'text-gray-600 '
                }`}
              >
                {item.col1}
              </td>
              ...
              <td
                className={`border text-center text-sm border-gray-100 px-4 py-2 ${
                  index >= itemsPerPage - 4 ? 'text-gray-400' : 'text-gray-500'
                }`}
              >
                {item.col4}
              </td>
              ...
              <td
                className={`border text-center text-sm border-gray-100 px-4 py-2 ${
                  index >= itemsPerPage - 4 ? 'text-blue-300' : 'text-blue-500 '
                }`}
              >
                {item.col6}
              </td>
